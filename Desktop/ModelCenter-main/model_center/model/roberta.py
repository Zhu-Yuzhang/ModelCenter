...
class RoertaLMHead(torch.nn.Module):
    ...
    def forward(self, hidden_states, input_embedding=None):
        ...
        if input_embedding is not None:
            logits = input_embedding.projection(
...
class Roberta(BaseModel):
    ...
    def forward(self,
        ...
        pkv_len = 0 if past_key_values is None else past_key_values[0][0].size(
            -2)
        ...
        with torch.no_grad():
            ...
            if attention_mask is not None:
                ...
                attention_mask = torch.arange(seq_length, device=device)[
                    None, :].repeat(batch, 1) < length[:, None]
            if attention_mask.dim() == 2:
                attention_mask = attention_mask.view(
                    batch, seq_length, 1) & attention_mask.view(batch, 1, seq_length)
            ...
            if token_type_ids is None:
                token_type_ids = torch.zeros(seq_length, dtype=torch.int32, device=device)[
                    None, :].repeat(batch, 1)
        ...
        token_type_embeds = self.token_type_embedding(
